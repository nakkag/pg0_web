...
- Limits are listed in `GET /api/agent/v1`; typical values: 5 s default timeout (30 s max), 10,000,000 statements, 128 MB of memory (`"memory_limit"` beyond that; arrays of more than a few hundred thousand elements do not fit), 200,000 characters of code, 1,000,000 characters of output, 2 concurrent runs (`429 too_many_runs` beyond that; the response carries `Retry-After: 1` and `retry_after_ms`, so wait a second and retry). While the server is short of free memory, runs answer `503 server_busy` (`Retry-After: 2`); wait and retry in the same way.
...
| `measureText(text, option = {})` | `{"width": w, "height": h}` in pixels; `option`: `{"fontsize", "fontface", "fontstyle"}`. **The browser returns the ink bounding box, not the advance width**: leading and trailing spaces are not counted (`"abc "` measures like `"abc"`), text made only of spaces is 0 × 0, and `height` is the height of the glyphs actually drawn (smaller than `fontsize`, depends on the letters). `drawText` does advance over spaces, so to place strings side by side measure the advance width instead: `measureText("|" + s + "|")["width"] - measureText("||")["width"]`, and use `fontsize` as the line height. Headless: like the browser, leading and trailing spaces are not counted and space-only text is 0 × 0; inner ASCII characters count 0.55 × fontsize (0.6 when `fontface` names a monospace font such as `"monospace"`), other characters 1 × fontsize, height = fontsize; deterministic, so layouts computed with a monospace font match the browser closely. |
...
| `inTouch()` | `{"x", "y", "touch": 0/1, "button": 0 left/1 middle/2 right, "pos": {{x, y}, ...}}`. When `touch` is 0, `x`/`y` keep the last position. `pos` holds all touch points (multi-touch). Like keys, a touch that starts and ends between two polls is not seen; automated browser tests should hold the pointer down for at least two frames (`mouse.down`, wait, `mouse.up`) instead of an instantaneous `click`/`tap`. |
| `inKey(key = none)` | No argument: array of held key names (JavaScript `KeyboardEvent.key`: `"ArrowLeft"`, `"a"`, `" "`, `"Enter"`). String: 1 if held (case-insensitive). Array: 1 only if all are held; names in the array must be lower case (`{"arrowleft", "a"}`). Browser: the held list is cleared 1 s after the last `keydown` event; since a key that stays down produces repeated `keydown` events through the OS key repeat (typically every 30 to 50 ms after an initial delay of about 250 to 500 ms), ordinary keys held down stay in the list and this is not a problem in practice. Modifier keys (Shift, Ctrl, Alt) do not repeat and therefore read as released after 1 s. Poll every frame and treat the list as "currently pressed". A key that is pressed and released between two polls (within one frame) is not seen at all, and two presses of the same key within one frame are seen as one (there is no key event queue); people hold keys for 50 to 100 ms so this is fine, but automated browser tests must hold a key for at least two frames (for example `keyboard.down`, wait 40 ms, `keyboard.up`) rather than use an instantaneous `press`. The headless `tap` stands for a press that lasts one whole frame. and while the screen is open every `keydown` is prevented from its browser default (Tab, Backspace, arrows, space, Ctrl+S and similar do not move focus, navigate, scroll or open dialogs; browser-level shortcuts such as Ctrl+W or F11 cannot be blocked). Text typed through an IME (Japanese input) arrives as the key `"Process"`, so only direct keys can be read; there is no mouse-wheel input. |
