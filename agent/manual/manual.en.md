...
- Limits are listed in `GET /api/agent/v1`; typical values: 5 s default timeout (30 s max), 10,000,000 statements, 256 MB of memory (`"memory_limit"` beyond that; arrays of millions of elements do not fit), 200,000 characters of code, 1,000,000 characters of output, 4 concurrent runs (`429 too_many_runs` beyond that; the response carries `Retry-After: 1` and `retry_after_ms`, so wait a second and retry).
