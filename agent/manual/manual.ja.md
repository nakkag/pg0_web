...
| `measureText(text, option = {})` | `{"width": w, "height": h}`（ピクセル）。`option`: `{"fontsize", "fontface", "fontstyle"}`。ヘッドレスでは ASCII 1 文字 0.55 × fontsize（`fontface` が `"monospace"` などの等幅フォントなら 0.6）、それ以外 1 × fontsize、高さ = fontsize の決定的な概算。等幅フォントで組んだレイアウトはブラウザとほぼ一致する。 |
