...
- 制限値は `GET /api/agent/v1` に載っています。標準値: タイムアウト 5 秒（最大 30 秒）、実行文数 10,000,000、メモリ 128MB（超えると `"memory_limit"`。数十万要素を超える配列は作れません）、コード 200,000 文字、出力 1,000,000 文字、同時実行 2（超えると `429 too_many_runs`。レスポンスに `Retry-After: 1` ヘッダと `retry_after_ms` が付くので、1 秒待って再試行）。サーバの空きメモリが足りないときは `503 server_busy`（`Retry-After: 2`）になるので、同様に待って再試行してください。
...
- `split_variable`: 最上位で作られていない変数を、あるブロックや関数で書き、別のブロックや関数で読んでいる（テキスト上の順序は問わない）。両者は別の変数になる。両方より前に最上位で作る。
...
- `global_overwrite`: 関数の中で `var` 無しに `for (i = 0; ...)` のようにループカウンタへ代入しているが、同じ名前の変数が最上位にもある。関数がグローバルの `i` を壊し、呼び出し元のループが狂います。関数内だけの変数は `var i` と宣言してください（グローバルの状態を意図して更新する代入には出ません）。
- `builtin_shadow`: 標準関数やライブラリ関数と同じ名前（大文字小文字を区別しない）の関数を定義している。元の関数が使えなくなります。
- `undeclared_variable`: `#option("strict")` があるのに、そのスコープにも外側にも `var` 宣言の無い変数を使っている（引数は宣言済み扱い）。インタプリタはその行を実行したときにしか報告しないが、チェックでは実行前に分かる。
...
| `sources` | object | `{}` | 追加のソースファイル `{"util.pg0": "コード", ...}`。プログラム（や他のソース）から `#import("util.pg0")` で取り込めます（`.pg0` は省略可。`/check` でも使えます）。保存せずに複数ファイルのプログラムを試すための仕組みで、3.12 参照。最大 30 個、各 200,000 文字。 |
...
- `( )`、`[ ]`、配列初期化子の `{ }` の中では改行は文の区切りになりません。閉じ括弧を次の行に置けます:
...
```
  ...
  ブロックの `{ }` は対象外です。ブロックの中では 1 行 1 文のままなので、続けたい式は括弧で囲みます（`x = (a + b` の次の行に `+ c)`）。
...
**整数**: 32bit 符号付き（-2,147,483,648 ～ 2,147,483,647）。演算はラップアラウンドします（`2147483647 + 1` は `-2147483648`）。範囲外のリテラルは範囲内に丸められ、`int`、`number`（整数の文字列）、`floor`、`ceil`、`round` の結果も同様に飽和します。範囲を超えうる合計は実数で計算し、オペランドに小数部を残してください。`a = b = 2000000000` のとき `(a + 0.5) + (b + 0.5) - 1` は `4000000000` になりますが、`a * 1.0 + b` は `2000000000 * 1.0` に小数部が無く整数に戻るため溢れます。リテラル: `1234`、`-123`、`0123`（8 進、PG0.5）、`0x12`（16 進、PG0.5）。指数表記（`1e3`）は使えません。
...
15. 指数リテラル、ブロックコメント、波括弧なしの `else`、三項演算子、文字列の添字はありません。逆に、できるのに迷いやすいもの: `var a = 1, b = 2`、`a[] = {}`（空の配列。`length(a)` は 0）、1 つの `case` の後に複数の文、配列を返す関数（`return {1, 2}`）、要素への配列の格納（`args[0] = {3, 4}` は入れ子になる）。
...
19. `( )`、`[ ]`、配列初期化子の `{ }` の中は自由に改行できます（`a[] = {1,\n 2,\n 3\n}`）。ただしブロックの `{ }` は別で、中では 1 行 1 文のままです。`x = a + b\n + c` は 2 つの文になり `x` は `a + b` です。続けるには `x = (a + b\n + c)` と括弧で囲みます。
...
23. `switch` の中の `continue` は外側のループの `continue` として働き（C と同じ）、`return` も `switch` の中で使えます。
24. `&` の参照引数には `a[i][j]` のような配列要素も渡せます（その要素そのものが参照されます）。再帰で多段の要素に書き込むときに便利です。
25. 配列初期化子の中の `{}`（空配列）や `-1` のような単項マイナスの式はキー無しの要素になります（17 の裸の変数だけがキー付き）。
26. 変数名は大文字小文字を区別しますが、配列のキーは区別しません（`b["K"]` と `b["k"]` は同じ要素）。変数名をキーにして値を管理するプログラムでは `A` と `a` が衝突します。
...
- 取り込んだスクリプトは `#import` の位置で読み込まれ、その最上位の文が取り込み元の残りより先に 1 回実行されます。以降、そのスクリプトの**関数**を取り込み元（および後から取り込まれた他の部品）から呼べます。**変数は見えません**。変数は部品のものであり、その部品の関数だけが使えるので、データは引数と戻り値で受け渡します（大きな配列は `&`）。
- 取り込みは入れ子にできます（部品が別の部品を取り込める）。直接と別の部品経由で 2 回取り込まれたスクリプトは 1 回だけ読み込まれます。取り込み中のスクリプトを再び取り込む（循環）とエラーになります。
- 関数名の解決順は、取り込み元自身の関数、取り込んだ部品（取り込み順）、ライブラリ関数の順です。同名の関数を定義すれば部品の関数を上書きできます。
- `#import` はライブラリの取り込みと同様にプログラムを PG0.5 にします。部品は単体で正しい PG0.5 である必要があり、部品内の構文エラーや実行時エラーは主プログラムの取り込み失敗「スクリプトまたはライブラリの読み込みに失敗しました」になり、メッセージに部品の cid と行が入ります。
- 非公開のスクリプトも取り込めます。必要なのは cid だけです。1 回の実行で取り込める保存済みスクリプトは 30 個までです。
- API では `/check`、`/run`、`/scripts/{cid}/run`、保存時の `check: true` のいずれも、その時点の保存済みスクリプトから取り込みを解決します。`PUT` で部品を更新すると、それを取り込む全プログラムに次の実行から反映されます。

**部品の公開範囲。** 他のシステムでも使えそうな共有部品（ベクトル演算、メニュー、テキストボックス、ソートなど）は**公開**（`"private": 0`）で保存し、内容が分かる名前と提供する関数を書いたメモを付けます。そのシステムの中でしか意味を持たない個別の部品（あるゲームのステージ、あるツールの画面など）は**非公開**（`"private": 1`）で保存し、公開一覧を見やすく保ちます。主プログラムは通常のスクリプトと同じように保存します。

**進め方。**

1. 部品ごとに一つの関心事の関数（と必要な状態）だけを書き、テスト用の呼び出しを末尾に付けて `/run` で動かし、保存します（`check: true`、インターフェースを説明したメモ）。
2. 主プログラムでは部品を cid で取り込み、初期化・ゲームループ・画面などのつなぎだけを書きます。
3. システム全体を `/run`（入力タイムライン、`variables`、`profile`）で試し、主プログラムを `check: true` で保存します。
4. 部品を変えるときは `PUT` でメモ付きで更新します。`GET /scripts/{cid}/history` でバージョンが見えます。主プログラムを再実行して壊れていないことを確かめます。
...
| `number(s: str)` | num | `s` の先頭から 10 進の整数か実数を読み取る（`"12abc"` は 12、`"abc"` は 0。指数表記は不可で `"3e9"` は 3）。32bit を超える整数の文字列は飽和する（`"3000000000"` は 2147483647）。実数として受け取りたいときは小数点を付ける（`"3000000000.0"` は実数の 3000000000）。 |
| `int(v: str\|num)` | int | 32bit 整数に変換（0 方向に切り捨て）。範囲外の値は 2147483647 / -2147483648 に飽和する。 |
| `code(s: str, i: int = 0)` | int | 位置 `i` の文字コード。範囲外は 0。 |
| `char(c: int)` | str | 文字コード `c` の 1 文字。 |
| `getKey(a: arr, i: int)` | str | 要素 `i` のキー（無ければ `""`）。`a["key"]` は読むだけで要素を作るので、作らずに存在を調べるには `getKey` を走査して（大文字小文字を区別せず）比較するか、キーの一覧を別の配列に持つ。削除の関数は無いので、要素を除いた配列を作り直す。 |
...
| `floor(n)`, `ceil(n)`, `round(n)` | 切り捨て、切り上げ、四捨五入（`round(-2.5)` は -2。JavaScript と同じ）。整数を返す。`int` と同様、32bit の範囲を超える結果は飽和する。 |
...
実数を小数点以下の桁数を決めて文字列にするには（実数の表示は 16 桁固定）、拡大して四捨五入し、文字列関数で小数点を入れ直します。約 35 ステップです: `s = "" + round(v * 100)` の後、`substring(s, 0, length(s) - 2) + "." + substring(s, length(s) - 2)` で `v = 3.14159` から `"3.14"` が得られます（`v < 1` のときは先に `s` の頭を 0 で埋める）。
...
| `wait(ms: int)` | Web エディタでは `ms` ミリ秒待ちます（停止ボタンで中断可）。API では待たず、合計が `stats.waited_ms` に入ります。`lib/screen.pg0` は不要で、フレームには数えません。コンソール向けプログラムの「実行速度」に使えます。 |
...
| `measureText(text, option = {})` | `{"width": w, "height": h}`（ピクセル）。`option`: `{"fontsize", "fontface", "fontstyle"}`。**ブラウザが返すのは送り幅ではなくインク（描かれた部分）の範囲**で、先頭と末尾の空白は数えられず（`"abc "` は `"abc"` と同じ幅）、空白だけの文字列は 0 × 0、`height` は実際に描かれた字形の高さ（`fontsize` より小さく、文字によって変わる）。`drawText` は空白の分も進めて描くので、文字列を横に並べるときは送り幅を測る: `measureText("|" + s + "|")["width"] - measureText("||")["width"]`。行の高さには `fontsize` を使う。ヘッドレスではブラウザと同様に先頭と末尾の空白を数えず、空白だけなら 0 × 0。内側の ASCII 1 文字 0.55 × fontsize（`fontface` が `"monospace"` などの等幅フォントなら 0.6）、それ以外 1 × fontsize、高さ = fontsize の決定的な概算。等幅フォントで組んだレイアウトはブラウザとほぼ一致する。 |
| `rgbToPoint(x, y)` | ピクセルの色 `{"r", "g", "b"}`（0～255）。ヘッドレスでは常に黒。 |
| `rgbToHex(rgb)` / `hexToRgb(hex)` | `{"r", "g", "b"}` と `"#rrggbb"` の相互変換。 |
| `inTouch()` | `{"x", "y", "touch": 0/1, "button": 0 左/1 中/2 右, "pos": {{x, y}, ...}}`。`touch` が 0 のとき `x`/`y` は最後の位置。`pos` は全タッチ点（マルチタッチ）。キーと同様、2 回の読み取りの間に始まって終わったタッチは見えないので、ブラウザの自動テストでは瞬間的な `click`/`tap` ではなくポインタを 2 フレーム以上押し続ける（`mouse.down`、待つ、`mouse.up`）。 |
| `inKey(key = なし)` | 引数なし: 押されているキー名の配列（JavaScript の `KeyboardEvent.key`: `"ArrowLeft"`, `"a"`, `" "`, `"Enter"`）。文字列: 押されていれば 1（大文字小文字を区別しない）。配列: 全て押されていれば 1。配列内の名前は小文字で書く（`{"arrowleft", "a"}`）。ブラウザでは最後の `keydown` イベントから 1 秒後に一覧が空になるが、押し続けたキーは OS のキーリピートで `keydown` が繰り返し発生する（一般に約 250～500ms の初期遅延の後、30～50ms 間隔）ためタイマーがその都度リセットされ、通常のキーを押し続けている間は一覧に残る。実用上は問題ない。Shift、Ctrl、Alt などの修飾キーはリピートしないため 1 秒後に離した扱いになる。毎フレーム読んで「今押されているキー」として扱う。2 回の読み取りの間（1 フレーム内）に押して離したキーはまったく見えず、同じキーを 1 フレームの間に 2 回押しても 1 回に見える（キーイベントのキューは無い）。人の操作は 50～100ms 押されるので問題ないが、ブラウザの自動テストでは瞬間的な `press` ではなく 2 フレーム以上押し続ける（例: `keyboard.down`、40ms 待つ、`keyboard.up`）。ヘッドレスの `tap` は 1 フレームまるごと押した状態を表す。画面が開いている間はすべての `keydown` のブラウザ既定動作が抑止される（Tab、Backspace、矢印、スペース、Ctrl+S などでフォーカス移動・戻る・スクロール・ダイアログは起きない。Ctrl+W や F11 などブラウザ自体のショートカットは抑止できない）。IME（日本語入力）経由の文字はキー名 `"Process"` になるため読めず、直接のキーだけが扱える。マウスホイールの入力は無い。 |
